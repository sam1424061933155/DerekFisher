...
inline void vertexSuimonoDisplace (inout appdata_full v){
	...
	float fr = tex2Dlod(_FlowMap, float4((tex.x)+_offsetShore.x,(tex.y)+_offsetShore.y,0.0,0.0)).r;
	...
	//normalize shoreline
	v.vertex.y = lerp(v.vertex.y,origY, (fr) * normalShore*_suimono_HeightProjection);
	
	//raise and warp wavetex at shoreline
	v.vertex.y += ((waveTex * _WaveShoreHeight) * (fr) *_suimono_HeightProjection);// * (1.0-s);
